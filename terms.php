...
<div class="container">
    <div class="row">
        ...
        <h3 class="text-left text-lead">Our Terms of Services</h3> 
        <hr>
        <?php
        $terms_page = "SELECT * FROM terms WHERE terms_id='1'";
        $terms = $db->select($terms_page);
        if($terms){   
        while($result = $terms->fetch_assoc()){
        ?>
